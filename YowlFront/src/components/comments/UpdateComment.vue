...
export default {
  ...
  methods: {
      handleComment(data) {
        const obj = {...data, user_id: this.$store.getters['auth/getAuthId'], id: parseInt(this.comment_id), post_id: parseInt(this.post_id) }
        this.loading = true;
        console.log(obj);
        this.$store.dispatch("comment/updateCommentItem", obj).then(
          () => {
